...
export default class ArticleGroup extends Component
{
  ...
  load()
  {
    ...
    axios.get(`${BaseHost}/reading/get_article_data.json?article_id=${id}`).then((res) => {
      console.log("res", res);
      that.setState({data: res.data.data,loading:false});
  ...
  render()
  {
    ...
    if(article && article.id)
    {
        ...
        var children_divs = children.map(c=>{
    ...
    let showView = <div style={{textAlign:"left"}}>
    ...
    return  this.state.loading == true ? <CLoading /> : showView
